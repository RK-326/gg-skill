...
- ШИРИНА картинок-иллюстраций разделов: на ВСЮ ширину текста, `cx="7223760"` EMU
  (как плашки). НЕ маленьким квадратом по центру. Соотношение сторон ~16:9 (баннер),
  высота пропорционально (`cy ≈ cx*9/16 ≈ 4063365`). Абзац с картинкой по центру (`jc center`).
